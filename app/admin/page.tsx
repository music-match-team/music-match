      ...
      <main className="flex-1 lg:ml-64 p-4 md:p-8 pt-20 lg:pt-8 min-w-0">
        ...
        <div className="mb-8">
          ...
          <p className="text-zinc-400 text-sm mt-1">
            Benvenuto, <span className="text-violet-400 font-semibold">{admin.username}</span>. Panoramica generale di MusicMatch.
          </p>
        </div>
          ...
          {statCards.map((card) => (
            ...
            >
              ...
              <p className="text-2xl font-extrabold text-white">
                {loading ? (
                  <span className="inline-block w-8 h-7 bg-zinc-700/50 rounded animate-pulse" />
                ) : (
                  card.value
                )}
              </p>
              <p className="text-xs text-zinc-400 font-medium mt-1">{card.label}</p>
            </Link>
          ))}
        </div>
        ...
        <div className="bg-zinc-900/50 border border-zinc-800 rounded-2xl p-6 shadow-xl">
          <div className="flex items-center justify-between mb-4">
            ...
            <Link href="/admin/segnalazioni" className="text-xs text-violet-400 hover:text-violet-300 font-semibold transition-colors hover:underline">
              Vedi tutte →
            </Link>
          </div>
          ...
          {loading ? (
            ...
            <p className="text-zinc-500 text-sm py-6 text-center">Nessuna segnalazione presente.</p>
          ) : (
                ...
                <div key={s.idSegnalazione} className="py-3 flex items-start justify-between gap-4">
                  <div>
                    <p className="text-sm text-zinc-200 font-medium">
                      <span className="text-zinc-400">Da</span> {s.mittente.username}{" "}
                      <span className="text-zinc-400">→</span>{" "}
                      <span className="text-amber-300">{s.destinatario.username}</span>
                    </p>
                    <p className="text-xs text-zinc-500 mt-0.5 line-clamp-1">{s.motivo}</p>
                  </div>
        ...
        <div className="mt-8 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
          {[
            ...
            { href: "/admin/utenti", label: "Cerca Utente", color: "bg-violet-600/10 border-violet-700/30 hover:bg-violet-600/20" },
            { href: "/admin/eventi", label: "Controlla Eventi", color: "bg-blue-600/10 border-blue-700/30 hover:bg-blue-600/20" },
          ].map((item) => (
            <Link
              key={item.href}
              href={item.href}
              className={`${item.color} border rounded-xl px-4 py-3 text-sm font-semibold text-zinc-200 transition-all text-center hover:scale-[1.02]`}
            >
              {item.label}
            </Link>
          ))}
